...
public class ProcessingEmployees {
    public static void main(String[] args) {
        ...
        // Creates a predicate that can be used in the filter method.
        // This predicate returns true for salaries in the range $4000-$6000
        Predicate<Employee> fourToSixThousand =  e -> (e.getSalary() >= 4000 && e.getSalary() <= 6000);

        Function<Employee, Double> salary = Employee::getSalary;

        // Display the Employees with salaries between $4000 and $6000 sorted 
        // into ascending order by salary amount
        ...
        // Functions for getting first and last names of Employees
        Function<Employee, String> byFirstName = Employee::getFirstName;
        Function<Employee, String> byLastName = Employee::getLastName;

        // Comparator for comparing Employees by last name, and then first namne
        Comparator<Employee> lastThenFirst = Comparator.comparing(byLastName)
                                                        .thenComparing(byFirstName);
        // sort employees by last name, then first name
        System.out.printf("%nEmployees om ascending order by last name then first:%n");
        list.stream()
            .sorted(lastThenFirst)
            .forEach(System.out::println);
        
        // sort employees in descending order by last name, then first name
        System.out.printf("%nEmployees om descending order by last name then first:%n");
        list.stream()
            .sorted(lastThenFirst.reversed())
            .forEach(System.out::println);

        // display unique employee last names sorted
        System.out.printf("%nUnique Last names:%n");
        list.stream()
