...
public class OpenHashMapTest {
	...
	protected HashMap<Integer, Character> hm;

	@Before
	public void setUp() {
		hm = new OpenHashMap<Integer, Character>();
	}
	...
	public void testPutGetNoResize() {
		...
		assertEquals("Incorrect capacity, should not have grown", 16, hm.capacity());
	}

	@Test
	public void testPutSameKeyReplacesValue() {
		hm.put(5, 'A');
		assertEquals("Did not find value for key: 5", (Character) 'A', hm.get(5));

		hm.put(5, 'B');
		assertEquals("Value for key: 5 was not replaced", (Character) 'B', hm.get(5));
	}
	...
	public void testHashSeverlKeysToSameHomeIndex() {
		...
		assertEquals("Did not find value for key: 5", (Character) 'A', hm.get(5));
		assertEquals("Did not find value for key: 20", (Character) 'B', hm.get(20));
		assertEquals("Did not find value for key: 36", (Character) 'C', hm.get(36));
		assertEquals("Did not find value for key: 52", (Character) 'D', hm.get(52));
	}
	...
	public void testResizesOnceAtRightSize() {
		...
		assertEquals("Incorrect capacity, should have doubled", 32, hm.capacity());
	}
	...
	public void testResizesSeveralTimes() {
		...
		for (int key : keys) {
			char val = hm.get(key);
			assertEquals("Incorrect value for key: " + key, ((key + 65) % 128), val);
		}

		assertEquals("Incorrect size", 513, hm.size());
		assertEquals("Incorrect capacity, should have increased several times", 1024, hm.capacity());
	}
	...
	public void testRemoveKeyInMap() {
		...
		for (int key : keys) {
			...
			assertEquals("Incorrect value for removed key: " + key, key + 65, val);
		}

		assertEquals("Incorrect size", 0, hm.size());
		assertEquals("Incorrect capacity, should not have changed", 16, hm.capacity());
	}
