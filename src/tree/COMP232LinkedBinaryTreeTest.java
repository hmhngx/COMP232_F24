...
import tree.COMP232LinkedBinaryTree.BTNode;


public class COMP232LinkedBinaryTreeTest {

    @Test
    public void testDefaultConstructor() {
        COMP232LinkedBinaryTree<String, Integer> tree = new COMP232LinkedBinaryTree<String,Integer>();
        assertEquals("Default constructor failed.", 0, tree.size());
    }

    @Test
    public void testRootNodeOnlyConstructor() {
        COMP232LinkedBinaryTree<String, String> tree = new COMP232LinkedBinaryTree<String, String>("Test", "1");
        assertEquals("Default constructor failed", 1, tree.size());
        assertNotNull("Key not contained in tree", tree.get("Test"));
        assertEquals("Incorrect value for key", "1", tree.get("Test"));
    }
	...
	public void testSubTreeConstructorEmptySubTrees() {
		COMP232LinkedBinaryTree<String, String> rTree = new COMP232LinkedBinaryTree<String, String>();
		COMP232LinkedBinaryTree<String, String> lTree = new COMP232LinkedBinaryTree<String, String>();
		COMP232LinkedBinaryTree<String, String> t = new COMP232LinkedBinaryTree<String, String>(
				lTree, "A", "1", rTree);
    ...
    /*
     * Here are some helper methods that traverses the tree and checks that 
     * every node is linked back to its proper parent
     */
    private void checkTreeStructure(BTNode<String, String> node) {
        if(node.left != null) {
            assertEquals("left child of " + node.key + " has incorrect parent pointer",
                        node, node.left.parent);
            checkTreeStructure(node.left);
        }
        if(node.right != null) {
            assertEquals("right child of " + node.key + " has incorrect parent pointer",
                        node, node.right.parent);
            checkTreeStructure(node.right);
        }
    }

    private void checkTreeKeysInOrder(BTNode<String, String> node, String exptKeys) {
        String treeKeys = getKeysInOrder(node);
        assertEquals("Keys are not in proper locations in in-order traversal", 
                    exptKeys, treeKeys);
    }
